...
/*
 * Robot "pip" - tiny wheeled robot with ultrasound sensors
 */
...
void setup() {
  Serial.begin(115200);
  Serial.println("Pip v1.0");

  obstacleDetector.center();
...
void setup_WiFi() {
  ...
  WiFi.hostname(Settings::APP_NAME);
  WiFi.mode(WIFI_STA);
...
void setup_OTA() {
  ...
  ArduinoOTA.setHostname(Settings::APP_NAME);
...
void setup_MQTT() {
  ...
  mqttClient.setClientId(Settings::APP_NAME);
